...
describe('resolveAuthProviderId', () => {
  ...
  it('selects auth0 when explicitly configured with credentials', () => {
    expect(resolveAuthProviderId({ configured: 'auth0', auth0Configured: true, workosConfigured: true })).toBe('auth0');
  });

  it('selects workos when configured with WorkOS credentials', () => {
    expect(resolveAuthProviderId({ configured: 'workos', auth0Configured: true, workosConfigured: true })).toBe(
      'workos',
    );
  });
  ...
  it('falls back to development when workos is selected but only Auth0 credentials exist', () => {
    ...
    expect(resolveAuthProviderId({ configured: 'workos', auth0Configured: true, workosConfigured: false })).toBe(
      'development',
    );
  });

  it('uses auth0 credentials when the configured value is unknown', () => {
    expect(resolveAuthProviderId({ configured: 'okta', auth0Configured: true, workosConfigured: false })).toBe('auth0');
  });

  it('uses workos credentials when the configured value is unknown but only WorkOS is set up', () => {
    expect(resolveAuthProviderId({ configured: 'okta', auth0Configured: false, workosConfigured: true })).toBe(
      'workos',
    );
  });
});
