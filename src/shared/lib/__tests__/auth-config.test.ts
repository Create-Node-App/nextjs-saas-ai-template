...
describe('session callbacks (provider-agnostic)', () => {
  ...
  function mockMemberships() {
    ...
    const { getAllTenantPermissionsForUser } = jest.requireMock('@/shared/lib/permissions') as {
...
describe('authorized callback route protection', () => {
  ...
  it.each([['/select-tenant'], ['/t/acme'], ['/t/acme/admin']])('requires authentication for %s', async (pathname) => {
